...
install() {
    ...
    echo "下一步 (Next steps):"
    echo ""
    echo "1. 重新載入 shell 設定 / Reload shell configuration:"
    echo "   ${BLUE}source $shell_config${NC}"
    echo ""
    echo "2. 或者重新啟動終端機 / Or restart your terminal"
    echo ""
    echo "3. 驗證安裝 / Verify installation:"
    echo "   ${BLUE}rm --version${NC}"
    echo ""
    echo "4. 查看使用說明 / View usage help:"
    echo "   ${BLUE}rm --help${NC}"
    echo ""
    echo "提示 (Tips):"
